...
class Inscripciones:
    ...
    def __init__(self, master=None):
        ...
        self.db_name = path.abspath(
            self.ruta_proyecto + "db/Inscripciones.db"
        )  # establecer la ruta de la db

        self.win = tk.Tk(master)

        # Establecer el icono de la ventana segun el sistema operativo
        if name == "nt":
            self.ruta_imagen = path.abspath(self.ruta_proyecto + "img/icono2.ico")
            self.win.iconbitmap(self.ruta_imagen)
        else:
        ...
        self.frm_1.configure(background="#A1A2AB", height=600, width=800)
        ...
        self.num_Inscripcion = ttk.Combobox(
            self.frm_1,
            name="num_inscripcion",
            state="readonly",
        ...
        self.num_Inscripcion.bind(
            "<<ComboboxSelected>>", self.info_Alum_For_Inscrp
        )  # Rellenar los campos segun el No. Inscripción

        # Label Fecha
        self.lblFecha = ttk.Label(self.frm_1, name="lblfecha")
        self.lblFecha.configure(background="#575669", foreground="white", text="Fecha:")
        self.lblFecha.place(anchor="nw", x=630, y=80)

        # Entry Fecha
        self.fecha = ttk.Entry(self.frm_1, name="fecha")
        self.fecha.configure(foreground="black", justify="center")
        ...
        self.lblIdAlumno.configure(
            background="#575669", foreground="white", text="Id Alumno:"
        )
        self.lblIdAlumno.place(anchor="nw", x=20, y=85)

        # Combobox Alumno
        self.id_Alum_Select = tk.StringVar()
        self.cmbx_Id_Alumno = ttk.Combobox(
            self.frm_1,
            name="cmbx_id_alumno",
        ...
        self.cmbx_Id_Alumno.bind(
            "<<ComboboxSelected>>",
            lambda event: (self.info_Alum(), self.asignacion_No_Inscripcion()),
        )  # Rellenar el No. Inscripción segun el Alumno
        ...
        self.lblNombres.configure(
            background="#575669", foreground="white", text="Nombre(s):"
        )
        self.lblNombres.place(anchor="nw", x=20, y=130)

        # Entry Alumno
        self.nombres = ttk.Entry(self.frm_1, name="nombres", state="readonly")
        self.nombres.configure(style="TEntry")
        self.nombres.place(anchor="nw", width=200, x=110, y=130)

        # Label Apellidos
        self.lblApellidos = ttk.Label(self.frm_1, name="lblapellidos")
        self.lblApellidos.configure(
            background="#575669", foreground="white", text="Apellido(s):"
        )
        ...
        self.lblIdCurso.configure(
            background="#575669", foreground="white", state="normal", text="Id Curso:"
        )
        ...
        self.lblDscCurso.configure(
            background="#575669", foreground="white", state="normal", text="Curso:"
        )
        ...
        self.button_style.theme_use("alt")
        self.button_style.theme_settings(
            "alt",
            {
                "TButton": {
                    "map": {
                        "background": [("active", "#4A4A55"), ("disabled", "#5A5A64")],
                        "foreground": [("active", "orange"), ("disabled", "#111111")],
                    },
                }
            },
        )  # cambia el fondo y color de los botones dependiendo de si su estado esta "normal" o "disabled"
        self.button_style.configure(
            "TButton", background="#3B3A4A", foreground="orange"
        )
        ...
        self.btnConsultar.configure(
            text="Consultar", command=lambda: self.opciones("S"), state="disabled"
        )
        self.btnConsultar.place(anchor="nw", width=100, x=670, y=180)

        # Botón Guardar
        self.btnGuardar = ttk.Button(self.frm_1, name="btnguardar")
        self.btnGuardar.configure(
            text="Guardar", command=lambda: self.opciones("G"), state="disabled"
        )
        self.btnGuardar.place(anchor="nw", x=200, y=240)

        # Botón Editar
        self.btnEditar = ttk.Button(self.frm_1, name="btneditar")
        self.btnEditar.configure(
            text="Editar", command=lambda: self.opciones("E"), state="disabled"
        )
        self.btnEditar.place(anchor="nw", x=300, y=240)

        # Botón Eliminar
        self.btnEliminar = ttk.Button(self.frm_1, name="btneliminar")
        self.btnEliminar.configure(
            text="Eliminar", command=lambda: self.opciones("D"), state="disabled"
        )
        ...
        self.btnCancelar.configure(
            text="Cancelar", command=lambda: self.opciones("C"), state="disabled"
        )
        self.btnCancelar.place(anchor="nw", x=500, y=240)

        # Botón Fecha de hoy
        self.btnFechaHoy = ttk.Button(self.frm_1, name="btnfechahoy")
        self.btnFechaHoy.configure(
            text="+",
            padding=(0, -5),
            command=self.fecha_Hoy,
            cursor="hand2",
        )
        ...
        style.configure(
            ...
            background="#3B3A4A",
            foreground="white",
            fieldbackground="#252330",
        )
        style.map("Treeview", background=[("selected", "#595168")])
        ...
        self.tView.column(
            "tV_descripción", anchor="w", stretch=True, width=200, minwidth=50
        )
    ...
    def serch_Id_Alum(self):
        ...
        if resultados:
            ...
            if (
                self.id_Alum_Select.get() != resultados[0][0] or len(resultados) != 1
    ...
    def info_Alum(self, *event):
        ...
        datos = self.run_query(
            f"SELECT Nombres, Apellidos FROM Alumnos WHERE Id_Alumno='{self.id_Alum_Select.get()}' ORDER BY Id_Alumno"
        )
    ...
    def cargar_tV(self, orden="Codigo_Curso", inscrito=False):
        ...
        if inscrito:
            ...
            busqueda = ""
            if self.id_Curso.get() != "" and self.descripc_Curso.get() != "":
                busqueda = f" AND Cursos.Codigo_Curso LIKE '%{self.id_Curso.get()}%' AND Descripcion LIKE  '%{self.descripc_Curso.get()}%' "
            elif self.id_Curso.get() != "":
                busqueda = f" AND Cursos.Codigo_Curso LIKE '%{self.id_Curso.get()}%' "
            elif self.descripc_Curso.get() != "":
                busqueda = f" AND Descripcion LIKE '%{self.descripc_Curso.get()}%' "
            ...
            query = f""" SELECT Cursos.*, Inscritos.Jornada FROM Cursos INNER JOIN Inscritos 
                    ON Inscritos.Codigo_Curso = Cursos.Codigo_Curso 
                    WHERE No_Inscripcion = '{self.num_Inscripcion.get()}' {busqueda} ORDER BY {orden}"""
            ...
            if len(cursos) != 0:
                ...
                for curso in cursos:
                    ...
                    self.tView.insert(
                        ...
                        "end",
                        text=curso[0],
                        values=(curso[1], curso[2], estado, jornada),
                    )
            ...
            query = f"""SELECT Cursos.* FROM Cursos LEFT JOIN Inscritos 
                    ON Cursos.Codigo_Curso = Inscritos.Codigo_Curso and Inscritos.No_Inscripcion = '{self.num_Inscripcion.get()}' 
                    WHERE Inscritos.Codigo_Curso IS NULL {busqueda} ORDER BY {orden}"""
            ...
            if self.id_Curso.get() != "" and self.descripc_Curso.get() != "":
                query += f" WHERE Codigo_Curso LIKE '%{self.id_Curso.get()}%' AND Descripcion LIKE '%{self.descripc_Curso.get()}%' "
            elif self.id_Curso.get() != "":
                query += f" WHERE Codigo_Curso LIKE '%{self.id_Curso.get()}%' "
            elif self.descripc_Curso.get() != "":
                query += f" WHERE Descripcion LIKE '%{self.descripc_Curso.get()}%' "
            ...
            for curso in cursos:
                ...
                jornada = "..."
                datos = self.run_query(
                    f"SELECT Codigo_Curso FROM Inscritos WHERE Id_Alumno='{self.id_Alum_Select.get()}'"
                )
                ...
                if len(datos) != 0:
                    for dato in datos:
                        if curso[0] == dato[0]:
                            estado = "Inscrito"
                            jornada = self.run_query(
                                f"SELECT Jornada FROM Inscritos WHERE Codigo_Curso='{curso[0]}' and Id_Alumno='{self.id_Alum_Select.get()}'"
                            )[0][0]
    ...
    def asignacion_No_Inscripcion(self):
        ...
        query = f"SELECT DISTINCT No_Inscripcion FROM Inscritos WHERE Id_Alumno = {self.id_Alum_Select.get()} ORDER BY No_Inscripcion DESC"
        inscripciones = self.run_query(query)

        if len(inscripciones) == 0:
            # configura los botones al cual el estudiante tiene acceso
            self.btnEliminar.configure(state="disabled", cursor="")  # no tiene acceso
            self.btnEditar.configure(state="disabled", cursor="")  # no tiene acceso
            self.btnConsultar.configure(state="disabled", cursor="")  # no tiene acceso
            self.btnGuardar.configure(state="normal", cursor="hand2")  # tiene acceso
            self.btnCancelar.configure(state="normal", cursor="hand2")  # tiene acceso

            no_Exis = self.run_query(
                "SELECT Valor FROM Variables_Numericas WHERE Nombre = 'No_Inscripciones'"
            )
            self.num_Inscripcion.set(f"¿{no_Exis[0][0] + 1}?")
        else:
            # configura los botones al cual el estudiante tiene acceso
            self.btnEliminar.configure(state="normal", cursor="hand2")  # tiene acceso
            self.btnEditar.configure(state="normal", cursor="hand2")  # tiene acceso
            self.btnConsultar.configure(state="normal", cursor="hand2")  # tiene acceso
            self.btnGuardar.configure(state="normal", cursor="hand2")  # tiene acceso
            self.btnCancelar.configure(state="normal", cursor="hand2")  # tiene acceso
    ...
    def cargar_No_Inscripciones(self):
        ...
        query = (
            "SELECT DISTINCT No_Inscripcion FROM Inscritos ORDER BY No_Inscripcion DESC"
        )
    ...
    def info_Alum_For_Inscrp(self, event):
        ...
        self.btnEliminar.configure(state="normal", cursor="hand2")
        self.btnEditar.configure(state="normal", cursor="hand2")
        self.btnConsultar.configure(state="normal", cursor="hand2")
        self.btnGuardar.configure(state="normal", cursor="hand2")
        self.btnCancelar.configure(state="normal", cursor="hand2")

        query = f"SELECT DISTINCT Id_Alumno FROM Inscritos WHERE No_Inscripcion='{self.num_Inscripcion.get()}'"
    ...
    def eleccion_jornada(self, id_Curso, curso):
        ...
        winEmergente.configure(background="#A1A2AB")
        ...
        winEmergente.protocol(
            "WM_DELETE_WINDOW", lambda: (interruptor.set(True), resultado.set(""))
        )

        frm = tk.Frame(winEmergente, name="frm_1")
        frm.configure(background="#30393E", height=160, width=300)
        ...
        btnCancelar.configure(
            text="Cancelar",
            command=lambda: (interruptor.set(True), resultado.set("")),
            cursor="hand2",
        )
    ...
    def guardar_botton(self):
        ...
        no_Inscripcion = self.num_Inscripcion.get().replace("¿", "").replace("?", "")
        ...
        if self.verificacion_fecha():
            ...
            if jornada:
                ...
                self.run_query(
                    f"INSERT INTO Inscritos (No_Inscripcion,Id_Alumno,Fecha_Inscripcion,Codigo_Curso,Jornada) VALUES ('{no_Inscripcion}','{id_alumno}','{fecha_inscripcion}','{codigo_curso}','{jornada}')"
                )
                ...
                max_no = self.run_query("SELECT MAX(No_Inscripcion) FROM Inscritos")
                ...
                if max_no[0][0] == int(no_Inscripcion):
                    ...
                    self.run_query(
                        f"UPDATE Variables_Numericas SET Valor = {no_Inscripcion} WHERE Nombre = 'No_Inscripciones'"
                    )
                    self.num_Inscripcion.set(no_Inscripcion)
    ...
    def cancelar_botton(self):
        ...
        # desabilitar botones
        self.btnGuardar.configure(state="disabled", cursor="")
        self.btnCancelar.configure(state="disabled", cursor="")
        self.btnEliminar.configure(state="disabled", cursor="")
        self.btnEditar.configure(state="disabled", cursor="")
        self.btnConsultar.configure(state="disabled", cursor="")
    ...
    def delete_botton(self):
        ...
        if current_item["text"] == "":
            ...
            query = f"SELECT * FROM Inscritos WHERE No_Inscripcion='{self.num_Inscripcion.get()}'"
            ...
            if len(existencia) == 0:
                ...
                if msg.askokcancel("Eliminar", f"""{texto1}\n\n{texto}"""):
                    ...
                    query = f"DELETE FROM Inscritos WHERE No_Inscripcion='{self.num_Inscripcion.get()}'"
            ...
            texto = (
                "ID: " + current_item["text"] + " | Curso: " + current_item["values"][0]
            ).center(60)
            if msg.askokcancel("Eliminar", f"""{texto1}\n\n{texto}"""):

                query = f"DELETE FROM Inscritos WHERE No_Inscripcion='{self.num_Inscripcion.get()}' and Codigo_Curso='{current_item['text']}'"
                self.run_query(query)
                msg.showinfo("Eliminar", "Eliminación exitosa")
                self.cargar_tV(inscrito=True)
        else:
    ...
    def editar_botton(self):
        ...
        if self.verificacion_fecha():
            ...
            mod = self.ventana_edicion(
                codigo_curso, descripc_curso, no_Inscripcion, jornada
            )

            if len(mod) == 1:
                self.run_query(
                    f"""UPDATE Inscritos SET Fecha_Inscripcion = '{fecha_inscripcion}', Jornada = '{mod[0]}' 
                                WHERE Codigo_Curso = '{codigo_curso}' and No_Inscripcion = '{no_Inscripcion}'"""
                )
                ...
                self.run_query(
                    f"""UPDATE Inscritos SET Codigo_Curso = '{mod[0]}', Fecha_Inscripcion = '{fecha_inscripcion}', Jornada = '{mod[1]}'
                               WHERE Codigo_Curso = '{codigo_curso}' and No_Inscripcion = '{no_Inscripcion}'"""
                )
    ...
    def ventana_edicion(
        ...
        frm.configure(background="#252330", height=75, width=620)
        ...
        lblCambioJornada.configure(
            background="#252330",
            foreground="white",
            state="normal",
            text="Cambio de jornada del curso seleccionado:",
        )
        lblCambioJornada.place(anchor="w", x=20, y=20)

        lblIdCurso = ttk.Label(frm, name="lblidcurso")
        lblIdCurso.configure(
            background="#252330", foreground="white", state="normal", text="Id Curso:"
        )
        ...
        lblDscCurso.configure(
            background="#252330", foreground="white", state="normal", text="Curso:"
        )
        ...
        lblJornada = ttk.Label(frm, name="lbljornada")
        lblJornada.configure(
            background="#252330", foreground="white", state="normal", text="Jornada:"
        )
        ...
        frm_2.configure(background="#06141B", height=75, width=620)

        cursos = self.run_query(
            f"""SELECT Cursos.Codigo_Curso, Cursos.Descripcion 
                              FROM Cursos LEFT JOIN Inscritos
                              ON Cursos.Codigo_Curso = Inscritos.Codigo_Curso and Inscritos.No_Inscripcion = '{no_Inscripcion_Seleccionado}'
                              WHERE Inscritos.Codigo_Curso IS NULL ORDER BY Cursos.Codigo_Curso"""
        )
        ...
        checkboxstyle.theme_use("alt")
        checkboxstyle.configure(
            "TCheckbutton", background="#11212D", foreground="orange"
        )
        ...
        checkbox = ttk.Checkbutton(
            frm_2,
            name="checkbox",
            text="Cambiar de curso inscrito.",
            variable=checkbox_value,
            command=lambda: (
                (
                    cmbx_Descrip_Curso.configure(state="readonly"),
                    cmbx_Jornada.configure(state="disabled"),
                    cmbx_Jornada_New.configure(state="readonly"),
                    cmbx_Id_Curso.configure(state="readonly"),
                    frm_2.configure(background="#06141B"),
                    lblCambioJornada.configure(background="#3B3A4A"),
                    lblDscCurso.configure(background="#3B3A4A"),
                    lblIdCurso.configure(background="#3B3A4A"),
                    lblJornada.configure(background="#3B3A4A"),
                    lblDscCursoNew.configure(background="#06141B", foreground="orange"),
                    lblIdCursoNew.configure(background="#06141B", foreground="orange"),
                    lblJornadaNew.configure(background="#06141B", foreground="orange"),
                    frm.configure(background="#3B3A4A"),
                )
                if checkbox_value.get()
                    ...
                    frm_2.configure(background="#06141B"),
                    lblCambioJornada.configure(background="#252330"),
                    lblDscCurso.configure(background="#252330"),
                    lblIdCurso.configure(background="#252330"),
                    lblJornada.configure(background="#252330"),
                    lblDscCursoNew.configure(background="#06141B"),
                    lblIdCursoNew.configure(background="#06141B"),
                    lblJornadaNew.configure(background="#06141B"),
                    frm.configure(background="#252330"),
                )
            ),
        )
        ...
        lblIdCursoNew.configure(
            background="#06141B", foreground="orange", state="normal", text="Id Curso:"
        )
        lblIdCursoNew.place(anchor="w", x=20, y=50)

        id_Curso_Select = tk.StringVar()
        cmbx_Id_Curso = ttk.Combobox(frm_2, name="id_curso")
        cmbx_Id_Curso.configure(
            justify="left",
            width=166,
            values=id_Cursos,
            state="disabled",
            textvariable=id_Curso_Select,
        )
        cmbx_Id_Curso.place(anchor="w", width=90, x=80, y=50)
        cmbx_Id_Curso.bind(
            "<<ComboboxSelected>>",
            lambda event: (
                descripc_Curso_Select.set(
                    descrip_Cursos[id_Cursos.index(cmbx_Id_Curso.get())]
                )
            ),
        )

        lblDscCursoNew = ttk.Label(frm_2, name="lbldsccurso")
        lblDscCursoNew.configure(
            background="#06141B", foreground="orange", state="normal", text="Curso:"
        )
        ...
        cmbx_Descrip_Curso.configure(
            justify="left",
            width=166,
            values=descrip_Cursos,
            state="disabled",
            textvariable=descripc_Curso_Select,
        )
        cmbx_Descrip_Curso.place(anchor="w", width=200, x=240, y=50)
        cmbx_Descrip_Curso.bind(
            "<<ComboboxSelected>>",
            lambda event: (
                id_Curso_Select.set(
                    id_Cursos[descrip_Cursos.index(cmbx_Descrip_Curso.get())]
                )
            ),
        )

        lblJornadaNew = ttk.Label(frm_2, name="lbljornada")
        lblJornadaNew.configure(
            background="#06141B", foreground="orange", state="normal", text="Jornada:"
        )
        lblJornadaNew.place(anchor="w", x=460, y=50)

        jornadas.append(jornada_Seleccionada)
        new_Jornada_New = tk.StringVar()
        cmbx_Jornada_New = ttk.Combobox(
            frm_2,
            name="cmbx_jornada",
            state="readonly",
            values=jornadas,
            textvariable=new_Jornada_New,
        )
        ...
        frm_3.configure(background="#253745", height=50, width=620)
        ...
        btnConfirmar.configure(
            ...
            command=lambda: (
                (
                    ...
                    if id_Curso_Select.get() != "" and new_Jornada_New.get() != ""
                    else msg.showerror("Error", "Por favor llene todos los campos")
                )
                if checkbox_value.get()
                else (
                    (
        ...
        btnCancelar.configure(
            text="Cancelar",
            command=lambda: (interruptor.set(True), resultado.set("cerrar")),
            cursor="hand2",
        )
