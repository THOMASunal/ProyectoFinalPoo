...
class Inscripciones:
    ...
    def __init__(self, master=None):
        ...
        self.db_name = path.abspath(self.ruta_proyecto + "db/Inscripciones.db")
        ...
        self.Buttonstyle.theme_use("alt")
        ...
        self.btnConsultar.configure(
            text="Consultar", command=lambda: self.opciones("S")
        )
        ...
        self.btnFechaHoy.configure(text="+", padding=(2, -5), command=self.fecha_Hoy)
        ...
        self.tView.place(anchor="nw", height=305, width=790, x=4, y=280)
        self.tView.bind("<Button-1>", self.tV_order)

        # Scrollbars
        self.scroll_H = ttk.Scrollbar(self.frm_1, name="scroll_h")
        self.scroll_H.configure(orient="horizontal", command=self.tView.xview)
        self.scroll_H.place(anchor="s", height=12, width=790, x=399, y=591)

        self.scroll_Y = ttk.Scrollbar(self.frm_1, name="scroll_y")
        self.scroll_Y.configure(orient="vertical", command=self.tView.yview)
        self.scroll_Y.place(anchor="s", height=300, width=12, x=788, y=580)
        self.tView.configure(
            xscrollcommand=self.scroll_H.set, yscrollcommand=self.scroll_Y.set
        )
    ...
    def eleccion_jornada(self, id_Curso, curso):
        ...
        # Establecer el icono de la ventana segun el sistema operativo
        if name == "nt":
            winEmergente.iconbitmap(self.ruta_imagen)
        else:
        ...
        btnCancelar.configure(
            text="Cancelar", command=lambda: (interruptor.set(True), resultado.set(""))
        )
    ...
    def ventana_edicion(
        ...
        # para cuando se cierra la ventana emergente se activa el interruptor
        winEmergente.protocol(
            "WM_DELETE_WINDOW", lambda: (interruptor.set(True), resultado.set("cerrar"))
        )
